...
`服务商` 在解析、分组、筛选和源数据落库前都会归一：

- 去掉 `(仓)`、`(配)`、`（仓）`、`（配）`。
- 如果服务商名称以 `-仓` 或 `-配` 结尾，也去掉这个结尾后缀。
- 再做首尾空格清理。

因此 `服务商A`、`服务商A(仓)`、`服务商A(配)`、`服务商A（仓）`、`服务商A（配）`、`服务商A-仓`、`服务商A-配` 会按同一个服务商处理并存储为 `服务商A`。

`QIC仓` 不做这类后缀归一，按文件中的仓名存储和分组。
...
```text
normalized_service_provider + qic_warehouse
```
...
BIC 源数据按 `accounting_period` 确保月分区。重跑同一个 BIC 任务时，会删除该任务旧的明细和源数据后重新解析写入。费用项允许列表或服务商名称归一规则变更后，历史结果不会自动变化，需要重跑受影响任务。

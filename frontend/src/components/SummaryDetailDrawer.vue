    ...
    <div class="detail-drawer">
      <div class="detail-toolbar">
        <div class="detail-context">
          <span class="context-item">数据年月：{{ context?.sourceDate || '-' }}</span>
          <span class="context-item">
            平台：
            <el-tag v-if="context?.platform" size="small" :class="getPlatformTagClass(context.platform)">
              {{ getPlatformLabel(context.platform) }}
            </el-tag>
            <template v-else>-</template>
          </span>
          <span class="context-item">店铺：{{ context?.shopName || '-' }}</span>
          <span class="context-item">汇总行数：{{ context?.summaryCount || 0 }}</span>
        </div>
      ...
      >
        ...
        <el-table-column prop="platform" label="平台" width="104" align="center">
          <template #default="{ row }">
            <el-tag size="small" :class="getPlatformTagClass(row.platform)">
...
import { exportSummaryExcel, getSummaryList, type SummaryRecord } from '@/api/summary'
...
async function handleExport(scope: 'all' | 'current_page') {
  ...
  const loadingRef = scope === 'all' ? exportAllLoading : exportCurrentPageLoading
...
:deep(.summary-detail-table .el-table__header),
:deep(.summary-detail-table .el-table__body),
:deep(.summary-detail-table .el-table__footer) {
  table-layout: fixed !important;
}

:deep(.summary-detail-table th.el-table__cell.is-right .cell) {

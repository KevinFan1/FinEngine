    ...
    <div class="page-container">
        ...
        <el-card shadow="never" class="table-card">
            ...
            >
                ...
                >
                    <template #default="{ row }">
                        <el-tag class="audit-type-pill" size="small">{{
...
const actionOptions = Object.entries(actionMap).map(([value, label]) => ({
    value,
    label,
}));
...
const activeFilterTags = computed<AuditFilterTag[]>(() => {
    ...
    if (searchForm.dateRange?.length === 2) {
        tags.push({
            key: "dateRange",
            label: "时间范围",
            value: `${searchForm.dateRange[0]} 至 ${searchForm.dateRange[1]}`,
        });
    }
    return tags;
});
...
function removeFilterTag(tag: AuditFilterTag) {
    if (tag.key === "modules") {
        searchForm.modules = searchForm.modules.filter(
            (module) => moduleLabel(module) !== tag.value,
        );
    } else if (tag.key === "actions") {
        searchForm.actions = searchForm.actions.filter(
            (action) => actionLabel(action) !== tag.value,
        );

    ...
    <div class="page-container checklist-page manual-edit-page">
        ...
        >
            <template #header>
                <div class="manual-edit-filter-head">
                    <span class="card-header-title">发票修改</span>
                </div>
            </template>
        ...
        <el-card shadow="never" class="table-card manual-edit-table-card">
                ...
                <div class="manual-edit-table-head">
                    <div class="summary-title-group">
                        <span class="card-header-title"
                            >发票修改命中 {{ matchedItems.length }} 条</span
                        >
                    </div>
            ...
            >
                ...
                <el-table-column label="收款商家" min-width="260">
                    <template #default="{ row }">
                        <el-input
                ...
                <el-table-column label="开票时间" width="220">
                    <template #default="{ row }">
                        <el-date-picker
                            v-model="row.invoice_time"
                            type="datetime"
                ...
                <el-table-column label="发票号码" min-width="240">
                    <template #default="{ row }">
                        <el-input
                            v-model="row.invoice_number"
                            placeholder="请输入发票号码"
                        />
                    </template>
                </el-table-column>
...
import {
    callbackReconciliationChecklistInvoiceEditsUpload,
    queryReconciliationChecklistInvoiceEdits,
    saveReconciliationChecklistInvoiceEdits,
    uploadReconciliationChecklistInvoiceEdits,
    type ReconciliationChecklistManualEditOssCredential,
    type ReconciliationChecklistInvoiceEditItem,
...
import {
    ...
    type InvoiceWorkbookRow,
} from "./manualEditWorkbook";

defineOptions({ name: "ReconciliationChecklistInvoiceEdits" });

type InvoiceEditPageState = {
    draft?: string;
    org_id?: number | null;
    matched_items?: ReconciliationChecklistInvoiceEditItem[];
...
const matchedItems = ref<InvoiceWorkbookRow[]>([]);
...
function persistPageState() {
    const state: InvoiceEditPageState = {
        draft: form.draft,
    ...
    sessionStorage.setItem(
        MANUAL_EDIT_STATE_KEYS.invoice,
        JSON.stringify(state),
    );
}

function restorePageState() {
    const raw = sessionStorage.getItem(MANUAL_EDIT_STATE_KEYS.invoice);
    if (!raw) {
        ensureDefaultOrgId();
        return;
    }
    try {
        const state = JSON.parse(raw) as InvoiceEditPageState;
...
function toInvoiceRow(item: Partial<InvoiceWorkbookRow>) {
    const row: InvoiceWorkbookRow = {
        ...
        receipt_merchant:
            typeof item?.receipt_merchant === "string"
                ...
                : "",
        invoice_time:
            typeof item?.invoice_time === "string" ? item.invoice_time : null,
        invoice_number:
...
async function handleQuery() {
    ...
    loading.value = true;
    try {
        const result = await queryReconciliationChecklistInvoiceEdits({
...
async function handleExport() {
    ...
    await exportManualEditWorkbook("invoice", matchedItems.value);
}
...
async function handleImportFileChange(event: Event) {
    ...
    try {
        await validateManualEditUploadHeaders(file, CHECKLIST_FILE_TYPE_INVOICE);
        const init = await uploadReconciliationChecklistInvoiceEdits({
            org_id: currentOrgId() as number,
        ...
        const task = await callbackReconciliationChecklistInvoiceEditsUpload({
            file_id: init.upload.file_id,
            oss_key: ossKey,
            file_size: file.size,
        });
        await ElMessageBox.alert(
            `发票修改文件已成功上传，任务 #${task.task_id} 将由任务中心继续处理。`,
            "上传成功",
            {

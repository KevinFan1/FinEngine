...
if [[ -z "$REMOTE_HOST" ]]; then
  echo "Missing DEV_HOST. Example:"
  echo "  DEV_HOST=dev.example.com bash frontend/deploy/deploy-dev.sh"
  exit 1
fi
...
echo "Dev frontend deployed to $REMOTE_USER@$REMOTE_HOST:$REMOTE_PATH"

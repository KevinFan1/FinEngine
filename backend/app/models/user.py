...
from sqlalchemy import BigInteger, Boolean, DateTime, ForeignKey, Index, SmallInteger, String, func, text
...
class User(SoftDeleteMixin, Base):
    ...
    must_change_password: Mapped[bool] = mapped_column(Boolean, default=False, server_default=text("false"), nullable=False, comment="首次登录后是否需要修改密码")
    role: Mapped[str] = mapped_column(String(20), default="member", comment="角色：超级管理员/组织管理员/普通成员")
    ...
    active_session_id: Mapped[str | None] = mapped_column(String(64), nullable=True, comment="当前有效登录会话 ID")
    active_session_ip: Mapped[str | None] = mapped_column(String(64), nullable=True, comment="当前登录 IP")

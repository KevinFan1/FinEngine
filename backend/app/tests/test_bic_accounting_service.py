...
from app.services.bic_accounting_service import BicAccountingService, _dedupe_bic_rows, _service_provider_filter, _upsert_source_rows_by_flow
...
def test_bic_reconciliation_workbook_contains_summary_and_shop_source_sheet() -> None:
    ...
    source_headers = [source_sheet.cell(row=1, column=index).value for index in range(1, 20)]
    source_values = [source_sheet.cell(row=2, column=index).value for index in range(1, 20)]
    ...
    assert source_headers == DOUYIN_BIC_HEADERS
    assert source_values[:8] == ["SETTLE-1", "ORDER-1", "REL-1", None, "质检费(通过)", "服务商A", "华东仓", 123.45]


def test_bic_reconciliation_workbook_dedupes_duplicate_shop_sheet_names() -> None:
    buffer = BicAccountingService._build_reconciliation_workbook(
        [
            {
                "shop_id": 5,
    ...
    assert workbook.sheetnames == ["汇总", "同名店铺", "同名店铺_2"]
    assert workbook["同名店铺"].cell(row=2, column=16).value == "FLOW-1"
    assert workbook["同名店铺_2"].cell(row=2, column=16).value == "FLOW-2"
...
def test_bic_source_rows_link_to_detail_rows() -> None:
    ...
    source_rows = BicAccountingService._build_source_rows(
        ...
        rows=[
            ...
            {"service_provider": "服务商A", "qic_warehouse": "华东仓", "amount": Decimal("2.30"), "fee_item": "质检费(通过)", "transaction_flow_no": "SCP-2"},
